...
install.packages("caret", dependencies=TRUE) #Classification And REgression Training package
#CARET provides a uniform interface to several machine learning algorithms and standardizes various other tasks such as Data splitting, Pre-processing, Feature selection, Variable importance estimation, etc.
install.packages('e1071', dependencies=TRUE) #for predict(), plot() and tune(functions)
install.packages('ROCR', dependencies=TRUE) #to plot ROC curve
...
#Read the file from local directory path, set above
mydata <- read.csv(file="sharks.csv", header=T, sep=",", strip.white = T, na.strings = c("NA","NaN","","?"))

#to get overview of data, we can also use str(mydata)
summary(mydata)
...
impute_data <- mice(mydata_levels[1:13])
...
###  Value Based Columns
#pmm means predictive mean matching (for numeric data)
